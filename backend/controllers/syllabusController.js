...
const uploadSyllabus = async (req, res) => {
  try {
    ...
    if (!courseName || !units || !units.length) {
      return res
        .status(400)
        .json({ message: "Course name and units are required" });
    }
    ...
    for (let u of units) {
      if (!u.unitName || !u.topics || !u.topics.length) {
        return res
          .status(400)
          .json({
            message: "Each unit must have a name and at least one topic",
          });
      }
    }
    ...
    res
      .status(201)
      .json({ success: true, message: "Syllabus uploaded", syllabus });
  } catch (error) {
    console.error(error);
